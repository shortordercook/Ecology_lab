...
Take a look at the data by clicking on it in the upper-right panel. There is a good amount of taxonomic information you will not need. For community data, it is useful to assign each taxonomic unit a unique taxon "code" to make calculations easier. For instance, some taxa I was able to take down to Genus, but others I kept at a coarser taxonomic resolution (Gastropoda for instance). Use the `code` column for all subsequent analyses.


## Calculations

Which stream is more biologically diverse (using richness-based measurements)? Since the primary difference between these two streams is higher nutrient concentrations from anthropogenic activity (primarily runoff from farming and pastureland), we can attribute any changes in the macroinvertebrate assemblage to this difference. 

Remember that `summarise()` compresses a table using some function. In the past we have used functions like `mean()` to generate summaries of the data for plotting. This time, we will use the function `n_distinct()`, which counts the number of unique things in a column. Pretty useful when our task is to count the number of unique taxa in a sample...

Alpha-diversity is the number of taxa in a sample (in this case a single kick-screen). What columns do we need to calculate that? Select those columns, use `group_by()` to perform calculations on your groups of interest, and then use summarise to collapse the information in the column that contains the taxa information. 
...
richness_diversity <- alpha_div %>%
  group_by(site) %>% # what are we still grouping by?
  summarise(mean_alpha = mean(alpha_richness)) %>% # take the mean of the richness values
  full_join(gamma_div, by = "site") %>% # this joins the two data frames together
  mutate(beta_richness = <>/<>) # how do you calculate beta-diversity?
```
