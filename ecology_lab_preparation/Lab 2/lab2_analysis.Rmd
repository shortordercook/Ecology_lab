...
After the t-test, we can summarise the respiration values for both organisms below for plotting.
...
gross_photosynthesis <- do_data %>%
  filter(TREATMENT == "dark") %>%
  mutate(GROSS_PHOTOSYNTHESIS = case_when(ORGANISM == "algae" ~ DELTA_DO_WEIGHT_HOUR + 1.818,
                                          ORGANISM == "macrophyte" ~ DELTA_DO_WEIGHT_HOUR + 1.841))
```
